# With weights and with particle visibility
...
# For physical edge pairs, calculate VRF once and apply to both directions
processed_edge_pairs_vrf = set()
...
for edge in E_vars.keys():
    ...
    # Skip if we've already processed this edge pair
    if edge_pair in processed_edge_pairs_vrf:
        continue
        
    processed_edge_pairs_vrf.add(edge_pair)
    ...
    visible_segments = set()
    for part_idx in edge_particle_visibility[(i, j)]:
        visible_segments.update(edge_particle_visibility[(i, j)][part_idx])
    
    # Calculate VRF and assign to both directions of the edge
    vrf_value = len(visible_segments) / (edge_length + epsilon)
    VRF[(i, j)] = vrf_value
    VRF[(j, i)] = vrf_value
