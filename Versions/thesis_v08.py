...
outer_buffered_polygon = polygon.buffer(30)  
...
xmin, ymin, xmax, ymax = outer_buffered_polygon.bounds
...
for x in range(int(xmin), int(xmax) + grid_spacing, grid_spacing):
    for y in range(int(ymin), int(ymax) + grid_spacing, grid_spacing):
        point = Point(x, y)
        if not polygon.contains(point) and buffered_polygon.exterior.distance(point) >= 0 and outer_buffered_polygon.contains(point):
    ...
    for j, p2 in enumerate(grid_points):
        if i != j:
            ...
            if dist <= max_edge_distance:
                ...
                if not buffered_polygon.intersects(edge_line):
